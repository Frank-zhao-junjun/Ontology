...
export function ManualGenerator({ onBack, selectedEntityId, relatedModels }: ManualGeneratorProps) {
  ...
  const autoGeneratedEntityIdRef = useRef<string | null>(null);
  ...
  useEffect(() => {
    if (
      ...
      !aiSuggestions &&
      !isGenerating &&
      autoGeneratedEntityIdRef.current !== selectedEntityId
    ) {
      autoGeneratedEntityIdRef.current = selectedEntityId;
      queueMicrotask(() => {
  ...
  }, [aiSuggestions, handleGenerateAI, isEntityMode, isGenerating, selectedEntityId]);
