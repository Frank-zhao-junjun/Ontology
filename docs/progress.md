...
| 工作项 | 说明 |
|--------|------|
| S18 严重缺陷修复 | `applyDerivedStepsToScenarioEpc` 改为保留现有 EPC 步骤并追加缺失推导步骤，避免“应用到 EPC”覆盖人工编辑步骤 |
| 回归测试 | `epc-derivation-store.spec.ts` 新增手工步骤保留用例；`pnpm run test:unit` 560/560 通过 |
