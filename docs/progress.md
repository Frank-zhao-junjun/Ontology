...
| 提交 | 说明 |
|------|------|
| `179e7a2` | fix: 修复 AI 建议应用丢写、实体删除孤儿引用、版本回滚跨项目/旧快照数据损坏 |
| `4c680d8` | fix: 补齐实体删除对规则条件、行为动作、EPC 子实体引用的级联清理 |
| `12bfcd8` | fix: 删除实体时清理 EPC 连接器中的失效活动与事件引用 |
| `cb8dc0a` | fix: 删除实体时清理实体引用属性与状态转换事件引用 |
