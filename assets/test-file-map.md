...
| UT-NORM-004 | `normalizer-aggregate-role.spec.ts` | 子实体父聚合校验 |
| UT-NORM-005 | `normalizer-event-guard.spec.ts` | 仅聚合根可发布领域事件 |
...
| IT-MODEL-ROLE-001 | `model-role-validation.spec.ts` | 子实体必须绑定父聚合 |
| IT-MODEL-ROLE-002 | `model-event-guard.spec.ts` | 非聚合根禁止配置/导出领域事件 |
...
| E2E-ROLE-001 | `modeling-child-entity-validation.e2e.spec.ts` | 子实体必须绑定父聚合 |
| E2E-ROLE-002 | `modeling-event-guard.e2e.spec.ts` | 非聚合根不能发布领域事件 |
...
│   ├── normalizer-aggregate-role.spec.ts
│   ├── normalizer-event-guard.spec.ts
...
│   ├── model-role-validation.spec.ts
│   ├── model-event-guard.spec.ts
...
└── e2e/
    ...
    ├── modeling-child-entity-validation.e2e.spec.ts
    ├── modeling-event-guard.e2e.spec.ts
